...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "A tablet-first remote control for OBS Studio and ProPresenter. Use either connection independently.",
      },
    ],
  }),
  component: Index,
...
function Index() {
  ...
  return (
    <main className="mx-auto flex min-h-[100dvh] max-w-[1400px] flex-col gap-3 p-3 sm:p-5">
      <header className="glass grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 rounded-2xl px-4 py-3 sm:flex sm:flex-wrap sm:justify-between">
        <div className="flex min-w-0 items-center gap-3">
          <div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl" style={{ background: "linear-gradient(135deg, var(--obs), var(--pp))" }}>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <Settings className="h-3.5 w-3.5" /> <span className="hidden sm:inline">Connections</span>
          </button>
        </div>
      </header>
      ...
      {anyOn ? (
        ...
        <div className="flex flex-1 flex-col items-center justify-center gap-6 px-4 py-10 text-center">
          <div className="relative">
            <div className="absolute inset-0 rounded-full blur-3xl" style={{ background: "color-mix(in oklab, var(--obs) 25%, transparent)" }} />
            <div className="relative grid h-16 w-16 place-items-center rounded-2xl" style={{ background: "linear-gradient(135deg, var(--obs), var(--pp))" }}>
          ...
          <div className="max-w-sm">
            ...
            <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
              Stage Deck works with either OBS Studio (WebSocket) or ProPresenter (Network API). Connect one or both to unlock the matching controls.
            </p>
          </div>
...
function ConnDot({ label, on, color }: { label: string; on: boolean; color: string }) {
  return (
    <span
      className="pill"
      style={{
        background: on ? `color-mix(in oklab, ${color} 18%, transparent)` : "color-mix(in oklab, white 6%, transparent)",
        color: on ? color : "var(--muted-foreground)",
      }}
    >
      <span className="dot" style={{ background: on ? color : "var(--muted-foreground)", boxShadow: on ? `0 0 8px ${color}` : undefined }} />
      {label}
    </span>
  );
}
