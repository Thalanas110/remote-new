...
import { ChevronLeft, ChevronRight, Square, Image, MessageSquare, Music, Layers, Presentation } from "lucide-react";
  ...
  return (
    <section className="glass flex h-full flex-col rounded-2xl p-4 sm:p-5">
      <header className="flex items-center justify-between gap-3">
        <div className="flex min-w-0 items-center gap-2.5">
          <div
            ...
            style={{ background: "color-mix(in oklab, var(--pp) 22%, transparent)", color: "var(--pp)" }}
          >
            <Presentation className="h-5 w-5" />
          </div>
          <div className="min-w-0">
            <h2 className="truncate text-sm font-semibold tracking-tight">ProPresenter</h2>
            <p className="truncate text-[11px] text-muted-foreground">{s.version || "REST API · :1025"}</p>
          </div>
        </div>
        <span
          className="pill"
          style={{
            background: s.connected ? "color-mix(in oklab, var(--pp) 18%, transparent)" : "color-mix(in oklab, white 6%, transparent)",
            color: s.connected ? "var(--pp)" : "var(--muted-foreground)",
          }}
        >
          <span className="dot" style={{ background: s.connected ? "var(--pp)" : "var(--muted-foreground)" }} />
      ...
      <div className="mt-4 rounded-xl border border-border p-4" style={{ background: "color-mix(in oklab, var(--pp) 8%, var(--card))" }}>
        ...
        <div className="mt-1 truncate text-base font-bold">{s.activePresentationName || (s.connected ? "—" : "Not connected")}</div>
      ...
      <div className="mt-3 grid grid-cols-2 gap-2">
        ...
        <button
          ...
          style={{ background: "linear-gradient(135deg, var(--pp), color-mix(in oklab, var(--pp) 60%, var(--primary)))" }}
        >
      ...
      <div className="mt-4">
        <h3 className="mb-2 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Clear Layers</h3>
        <div className="grid grid-cols-3 gap-2 sm:grid-cols-3">
          <ClearBtn onClick={call(() => ppClient.clearSlide())} disabled={offline} icon={<Layers className="h-4 w-4" />} label="Slide" />
          <ClearBtn onClick={call(() => ppClient.clearProps())} disabled={offline} icon={<Image className="h-4 w-4" />} label="Props" />
          <ClearBtn onClick={call(() => ppClient.clearMessages())} disabled={offline} icon={<MessageSquare className="h-4 w-4" />} label="Msgs" />
          <ClearBtn onClick={call(() => ppClient.clearAudio())} disabled={offline} icon={<Music className="h-4 w-4" />} label="Audio" />
          <ClearBtn onClick={call(() => ppClient.clearAnnouncements())} disabled={offline} icon={<MessageSquare className="h-4 w-4" />} label="Annc" />
          <button
      ...
      <div className="mt-4">
        <h3 className="mb-2 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Timer (by id/name)</h3>
...
function ClearBtn({ onClick, disabled, icon, label }: { onClick: () => void; disabled: boolean; icon: React.ReactNode; label: string }) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
...
function TimerControls({ disabled }: { disabled: boolean }) {
  ...
  return (
    <div className="flex gap-2">
      ...
      />
      <button onClick={call(() => ppClient.timerStart(id))} disabled={disabled || !id} className="btn-tap rounded-xl border border-border px-3 py-2 text-xs font-semibold disabled:opacity-40" style={{ color: "var(--pp)" }}>Start</button>
      <button onClick={call(() => ppClient.timerStop(id))} disabled={disabled || !id} className="btn-tap rounded-xl border border-border px-3 py-2 text-xs font-semibold disabled:opacity-40">Stop</button>
      <button onClick={call(() => ppClient.timerReset(id))} disabled={disabled || !id} className="btn-tap rounded-xl border border-border px-3 py-2 text-xs font-semibold disabled:opacity-40">Reset</button>
    </div>
  );
}
